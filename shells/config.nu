...
if (($env.SHELLS_OS in ["linux" "wsl"]) and ("/usr/lib/x86_64-linux-gnu" | path exists)) {
    let p = "/usr/lib/x86_64-linux-gnu"
...
def shells-regen-aliases [] {
    ...
    let outfile = ($nu.cache-dir | path join "shells" "aliases.nu")
...
do {
    ...
    let c = ($nu.cache-dir | path join "shells" "aliases.nu")
...
# =============================================================================
# SECTION 12 — Source the ~/.aliases cache (parse-time)
# =============================================================================
# This MUST be the last statement — nu parses top-to-bottom and we want all
# preceding defs/aliases registered before user aliases (which may reference
# them). The cache file is generated by `shells-regen-aliases`; bootstrap
# (empty file) is required before first source — see the header.
const __SHELLS_ALIAS_CACHE = ($nu.cache-dir | path join "shells" "aliases.nu")
source $__SHELLS_ALIAS_CACHE
