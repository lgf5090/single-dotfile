...
# ---- grep family ------------------------------------------------------------
if ($__shells_grep)
{ Set-Item function:global:grep  -Value ([scriptblock]::Create("& '$__shells_grep' --color=auto `@args"))
}
if ($__shells_fgrep)
{ Set-Item function:global:fgrep -Value ([scriptblock]::Create("& '$__shells_fgrep' --color=auto `@args"))
}
if ($__shells_egrep)
{ Set-Item function:global:egrep -Value ([scriptblock]::Create("& '$__shells_egrep' --color=auto `@args"))
}
...
switch ($SHELLS_OS)
{
    ...
    { $_ -eq 'macos' -or $_ -eq 'freebsd' }
    {
        function global:localip
        {
            ...
            if (-not $ip)
            { $ip = ipconfig getifaddr en1 2>$null
            }
            $ip
        }
        function global:ports
