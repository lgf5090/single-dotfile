...
if (Get-Command Set-PSReadLineOption -ErrorAction SilentlyContinue) {
    ...
    try { Set-PSReadLineOption -HistoryNoDuplicates } catch { }
...
if (-not $env:MSYS) { $env:MSYS = 'winsymlinks:nativestrict' }
...
# ---- Node.js ecosystem ------------------------------------------------------
if (-not $env:NPM_CONFIG_PREFIX) { $env:NPM_CONFIG_PREFIX = "$env:APPDATA\npm" }
...
if ([System.IO.Directory]::Exists("$HOME\.pyenv-win")) { $env:PYENV       = "$HOME\.pyenv-win" }
...
# Python ecosystem
__shells_prepend_dir `
    $(if ($env:PYENV)         { "$env:PYENV\pyenv-win\bin" }) `
    $(if ($env:PYENV)         { "$env:PYENV\pyenv-win\shims" }) `
    $(if ($env:ANACONDA_HOME) { $env:ANACONDA_HOME }) `
    $(if ($env:ANACONDA_HOME) { "$env:ANACONDA_HOME\Scripts" }) `
...
function global:lt { Get-ChildItem -Force @args | Sort-Object LastWriteTime }
...
if (-not $env:SHELLS_NO_PROMPT) {
    if (Get-Command starship -ErrorAction SilentlyContinue) {
        ...
        $global:__shells_user    = if ($env:USERNAME) { $env:USERNAME } else { $env:USER }
